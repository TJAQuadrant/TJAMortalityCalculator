...
# Try multiple candidate paths — Streamlit Cloud resolves __file__ differently
def _find_file(name):
    candidates = [
        os.path.join(MODEL_DIR, name),   # same dir as script
        os.path.join(os.getcwd(), name), # working directory
...
_css_path = _find_file("style.css")
if _css_path:
...
def load_artifacts():
    model      = joblib.load(_find_file("mort1yr_model_fitted_v7.joblib"))
    calibrator = joblib.load(_find_file("mort1yr_calibrator_v7.joblib"))
    with open(_find_file("mort1yr_imputer_fills_v7.json")) as f:
        fills = json.load(f)
    with open(_find_file("mort1yr_features_resolved_v7_nocosi.json")) as f:
...
with col_form:
    ...
    # All 8 labs use number_input with built-in label for uniform alignment.
    # Inactive gated labs render as disabled with a descriptive placeholder
    # so every column has the same structure: label → input box, no extra markup.
    ...
    with lc2:
        if has_anemia_now:
            hemoglobin = st.number_input("Hemoglobin (g/dL)", min_value=5.0, max_value=20.0,
                                         value=None, placeholder="e.g. 13.5", on_change=mark_dirty)
        else:
            st.number_input("Hemoglobin (g/dL)", min_value=5.0, max_value=20.0, value=None,
                             placeholder='Enable "Anemia" to activate', disabled=True, key="hgb_dis")
            hemoglobin = None
    with lc3:
    ...
    with lc6:
        if has_liver_now:
            albumin = st.number_input("Albumin (g/dL)", min_value=1.0, max_value=6.0,
                                       value=None, placeholder="e.g. 4.0", on_change=mark_dirty)
        else:
            st.number_input("Albumin (g/dL)", min_value=1.0, max_value=6.0, value=None,
                             placeholder='Enable "Liver disease" to activate', disabled=True, key="alb_dis")
            albumin = None
    with lc7:
        if has_diabetes_now:
            ...
            st.number_input("HbA1c (%)", min_value=4.0, max_value=14.0, value=None,
                             placeholder='Enable "Diabetes" to activate', disabled=True, key="hba1c_dis")
            hba1c = None
    with lc8:
        if has_anticoag_now:
            inr = st.number_input("INR", min_value=0.5, max_value=10.0,
                                   value=None, placeholder="e.g. 1.1", on_change=mark_dirty)
        else:
            st.number_input("INR", min_value=0.5, max_value=10.0, value=None,
                             placeholder='Enable "Anticoagulation" to activate', disabled=True, key="inr_dis")
            inr = None
    ...
    with st.expander("Reference: Predictor Importance & Clinical Interpretation  —  ranked by SHAP value"):
        _ref_path = _find_file("mort1yr_reftable.html")
        if _ref_path:
            with open(_ref_path, encoding="utf-8") as _rf:
                st.markdown(_rf.read(), unsafe_allow_html=True)
        else:
            st.caption("mort1yr_reftable.html not found — place it in the same directory as the app.")
